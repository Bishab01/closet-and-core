...
### Backend
1. Copy this example .env file content and fill in your DB credentials (should match your MYSQL configuration):
   ```
    DB_HOST=
    DB_NAME=
    DB_USER=
    ...
    DB_PORT = 
   ```
2. Serve the backend/ folder with PHP usig XAMPP. Place the backend/ folder inside htdocs/, start Apache and MySQL from the XAMPP control panel,

### Database
1. Create the database (same name as DB_NAME) and import backend/onlineStore.sql via phpMyAdmin.

### Frontend
1. `cd frontend && npm install`
2. Set `VITE_API_URL` in `.env` to point at your backend's `api/` folder.
3. Dev: `npm run dev`
